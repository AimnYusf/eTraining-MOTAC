...
$(function () {
  ...
  if (dt_table.length) {
    table = dt_table.DataTable({
      ...
      columns: [{ data: 'jab_id' }, { data: 'jab_ketpenu' }, { data: 'jab_ketring' }, { data: '' }],
      columnDefs: [
        {
          targets: 0,
          searchable: false,
    ...
    FormValidation.formValidation(crudForm, {
      fields: {
        jab_ketpenu: {
          validators: {
            notEmpty: {
              message: 'Sila masukkan nama jabatan'
            }
          }
        },
        jab_ketring: {
        ...
        instance.on('core.form.valid', function () {
          $.ajax({
            ...
            success: () => {
              Swal.fire({
                icon: 'success',
                title: $('#jab_id').val() == '' ? 'Berjaya Ditambah!' : 'Berjaya Dikemaskini!',
                customClass: {
                  title: 'm-0',
  ...
  $(document).on('click', '.edit-record', function () {
    const jab_id = $(this).data('id');

    $.ajax({
      url: `/tetapan/jabatan/${jab_id}`,
      type: 'GET',
      success: function (data) {
        $('#jab_tajuk').html('Edit Jabatan');
        $('#jab_id').val(data.jab_id);
        $('#jab_ketpenu').val(data.jab_ketpenu);
        $('#jab_ketring').val(data.jab_ketring);
        $('#crudModal').modal('show');
      }
    });
  });
