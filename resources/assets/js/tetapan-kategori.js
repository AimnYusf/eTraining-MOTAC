...
$(function () {
  ...
  if (dt_table.length) {
    table = dt_table.DataTable({
      ajax: {
        url: '/tetapan/kategori',
        type: 'GET'
      },
      columns: [{ data: 'kat_id' }, { data: 'kat_keterangan' }, { data: '' }],
      columnDefs: [
        {
          targets: 0,
          searchable: false,
        ...
        instance.on('core.form.valid', function () {
          $.ajax({
            ...
            success: () => {
              Swal.fire({
                icon: 'success',
                title: $('#kat_id').val() == '' ? 'Berjaya Ditambah!' : 'Berjaya Dikemaskini!',
                customClass: {
                  title: 'm-0',
  ...
  $(document).on('click', '.edit-record', function () {
    const kat_id = $(this).data('id');

    $.ajax({
      url: `/tetapan/kategori/${kat_id}`,
      type: 'GET',
      success: function (data) {
        $('#kat_tajuk').html('Edit Kategori');
        $('#kat_id').val(data.kat_id);
        $('#kat_keterangan').val(data.kat_keterangan);
        $('#crudModal').modal('show');
      }
    });
  });
