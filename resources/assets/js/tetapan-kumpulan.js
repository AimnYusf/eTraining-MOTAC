...
$(function () {
  ...
  if (dt_table.length) {
    table = dt_table.DataTable({
      ajax: {
        url: '/tetapan/kumpulan',
        type: 'GET'
      },
      columns: [{ data: 'kum_id' }, { data: 'kum_ketpenu' }, { data: '' }],
      columnDefs: [
        {
          targets: 0,
          searchable: false,
      ...
      buttons: [
        {
          ...
          action: () => {
            $('#crudForm')[0].reset();
            $('#kum_id').val(null);
            $('#kum_tajuk').html('Tambah Kumpulan');
    ...
    FormValidation.formValidation(crudForm, {
      fields: {
        kum_ketpenu: {
          validators: {
            notEmpty: {
              message: 'Sila masukkan kumpulan'
            }
          }
        }
      },
        ...
        instance.on('core.form.valid', function () {
          $.ajax({
            ...
            success: () => {
              Swal.fire({
                icon: 'success',
                title: $('#kum_id').val() == '' ? 'Berjaya Ditambah!' : 'Berjaya Dikemaskini!',
                customClass: {
                  title: 'm-0',
  ...
  $(document).on('click', '.edit-record', function () {
    const kum_id = $(this).data('id');

    $.ajax({
      url: `/tetapan/kumpulan/${kum_id}`,
      type: 'GET',
      success: function (data) {
        $('#kum_tajuk').html('Edit Kumpulan');
        $('#kum_id').val(data.kum_id);
        $('#kum_ketpenu').val(data.kum_ketpenu);
        $('#kum_ketring').val(data.kum_ketring);
        $('#crudModal').modal('show');
      }
    });
  });
